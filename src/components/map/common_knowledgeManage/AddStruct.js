...
class AddStruct extends React.Component {
  static contextTypes = {
  ...
  batchAddMapStruct = () => {
    if (this.state.display_batchAddMapStruct == 'block') {
      ...
      let ajaxTimeOut = $.ajax({
        ...
        success: function (data) {
          if (data.errorcode == '0') {
            PubSub.publish('BatchAddTopMapStructSuccess'); this.success()
            console.log('批量导入地图结构成功'); this.success()
          }
          else {
...
AddStruct = Form.create()(AddStruct);
