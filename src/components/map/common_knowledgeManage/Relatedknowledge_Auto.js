...
class Relatedknowledge_Auto extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  //获取自动关联知识点列表
  Relatedknowledge_AutoGet() {
    this.setState({
      checkedNode: [],
      checkedKeys: []
    });
    console.log('获取自动关联知识点列表-父节点');
    console.log(this.state.parent_know);
    let ajaxTimeOut = $.ajax({
      ...
      timeout: 2000,
      data: {
        "rcontent": this.state.parent_know //根据父节点自动关联知识点
      },
      success: function (data) {
        if (data.erroCode == "0") {
          ...
          let Msg = []
          console.log("data.msg555555555555555555555")
          console.log(data.msg)
          data.msg.forEach(function (item) {
            Msg.push({ "knowid": item.knowid, "pre_knowid": 0, "isLeaf": true, "title": item.title });
          })
          this.setState({
            treeData_auto: Msg
          });
          ...
          console.log("this.state.treeData9999999999999999999999")
  ...
  render() {
    ...
    //搜索结果_自动
    const SearchknowledgeResult_auto = this.state.SearchknowledgeResult_auto.map((v, i) => {
      if (v.title == '暂无可关联数据') {
        return (
          <p style={{ paddingTop: '50px', fontSize: '20px', paddingLeft: '50px' }} >暂无可关联数据</p>
        );
    ...
    // 关联按钮_自动
    const RelatedButton_auto = this.state.RelatedButton_auto.map((v, i) => {
      if (v.title == '可关联') {
        return (
          <Button type="primary" htmlType="submit" onClick={this.addKnowledgeRelations.bind(this)}>关联</Button>
        );
      ...
      <div >
        <Card style={{ height: '800px' }}>
          <Tabs defaultActiveKey="1">
            <TabPane tab={<span><Icon type="search" />手动关联</span>} key="1">
            ...
            <TabPane tab={<span><Icon type="search" />自动关联</span>} key="2">
                ...
                <Card style={{ height: '1750px' }} >
                  <div style={{ paddingTop: '10px' }}>
                    <div style={{ paddingTop: '50px' }}>
                      ...
                      // selectedKeys={this.state.selectedKeys}
                      >
                        {this.renderTreeNodes(this.state.treeData_auto)}
                      </Tree>
                      {RelatedButton_auto}
                    </div>
                  </div>
...
Relatedknowledge_Auto = Form.create()(Relatedknowledge_Auto);
