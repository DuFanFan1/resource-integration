...
class MapType_MyMap extends Component {
  static contextTypes = {
  ...
  deleteConfirm = () => {
    const { mapInfo } = this.props;
    console.log('我的地图' + mapInfo.mapContent)
    if (mapInfo.mapContent == true) {
      this.showConfirm();
    }
    else {
                  ...
                  <Row>
                    <Col span={23}>
                      <div style={{ float: 'right',display: this.state.display_name }}>
                        <span style={{ paddingLeft: '5px' }}><Button style={buttoncolor} size="large" onClick={this.editConfirm.bind(this)}>修改我的地图</Button></span>
                        <span style={{ paddingLeft: '5px' }}> <Button style={buttoncolor} size="large" onClick={this.deleteConfirm.bind(this)}>删除我的地图</Button></span>
                      </div>
                    </Col>
